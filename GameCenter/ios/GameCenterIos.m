...
+ (void)gcPostAchievement:(NSDictionary*)dict //(const char*)idName percent:(NSNumber*)percentComplete
{
    const char*idName = [[dict objectForKey:@"idName"] UTF8String];
    NSNumber* percentComplete = [dict objectForKey:@"percentComplete"] ;
    [[GameCenterIos shared]postAchievement:idName percent:percentComplete];
}
...
{
    NSString* idName = [dict objectForKey:@"idName"];
    return [[GameCenterIos shared]showScores :idName];
}
...
+ (void)gcPostScore:(NSDictionary*)dict
{
    ...
    [[GameCenterIos shared] postScore:[idName UTF8String]  score:score];
}
